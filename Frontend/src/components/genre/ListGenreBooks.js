...
class ListGenreBook extends Component {
    constructor(props) {
        super(props)
        this.state = {
            books: [],
    ...
    componentDidMount(){
        BookService.getBooksByGenre(this.state.genreId).then((res) => {
    ...
    render() {
        ...
        const sortBooks = (currentBooks, ascending) => {
            return currentBooks.sort((bookA, bookB) => {
              if (ascending) {
                return bookA.bookId > bookB.bookId ? 1 : -1;
              } else {
                return bookA.bookId < bookB.bookId ? 1 : -1;
              }
            }); 
        ...
        return (
            ...
            {currentUser &&  (currentUser.roles[0] === "ROLE_MODERATOR" || currentUser.roles[1] === "ROLE_MODERATOR" ) && (
                ...
                <div style={{ marginLeft: "0px", marginRight: "10px", marginTop: "-40px" ,marginBottom: "10px"}} className=" card-columns grid "> 
